...
(function ($) {
  ...
  */
   ...
   */
	...
	function articleSlider(context) {

		var $context = $('' + context),
		    slider = $('.article-external-slide', $context);

		slider.on('initialized.owl.carousel', function (event) {
			$('.article-external-info__number-slides', $context).text(event.item.count);
		});
		...
		slider.owlCarousel({
			...
			dots: false,
			items: 1,
			navClass: ['article-external-info__arrow-left', 'article-external-info__arrow-right'],
			navContainer: context + ' .article-external-info__block-title'
		});

		slider.on('changed.owl.carousel', function (event) {
			$('.article-external-info__current-slide-number', $context).text(event.item.index + 1);
		});
	}

	articleSlider('#article-external-info-1');
	articleSlider('#article-external-info-2');
